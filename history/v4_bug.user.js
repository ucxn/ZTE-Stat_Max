// ==UserScript==
// @name         中兴路由器(ZTE) 赛博极客增强版 (宽屏高频终极版)
...
(function() {
    ...
    const CONFIG = {
        ...
        portMap: {          // 物理/虚拟端口中文映射字典 (根据需要随意修改)
            "eth1": "端口 1",
            "eth2": "端口 2",
            "eth3": "端口 3",
            "eth4": "端口 4",
    ...
    // ======== [2] 注入极客宽屏 CSS (完美复刻图纸版) ========

    const style = document.createElement('style');
    style.innerHTML = `
        /* 清除原厂浮动，建立严格的 Flexbox 网格体系 */
        .config-item { clear: both; }
        .config-item-box { display: flex !important; align-items: stretch !important; padding-bottom: 10px !important; }
        .config-item .logo, .config-item .info, .config-item .speed { float: none !important; display: flex !important; flex-direction: column; }

        .config-item .logo { width: 33% !important; flex-direction: row !important; align-items: stretch; }
        .config-item .dev-logo { flex-shrink: 0; }
        .config-item .dev-intro { flex: 1; display: flex; flex-direction: column; min-width: 0; }

        .config-item .info { width: 27% !important; padding: 0 10px !important; }
        .config-item .speed { width: 40% !important; justify-content: center; padding: 0 10px !important; }
        ...
        /* 左侧：上行流量条 (推到底部) */
        .gege-up-box { margin-top: auto !important; width: 95%; margin-bottom: 2px; }
        /* 中间：雷达条与下行流量条 (推到底部) */
        .gege-ratio-box { margin-top: auto !important; width: 95%; margin-bottom: 4px; }
        .gege-down-box { margin-top: 0 !important; width: 95%; margin-bottom: 2px; }

        /* 共用微型条排版 */
        .t-row { font-size: 12px; font-weight: bold; margin-bottom: 3px; display: flex; justify-content: space-between; }
        ...
        deviceItems.forEach(item => {
            ...
            if (devIntro) {
                ...
                if (!upBox) {
                    ...
                    upBox.innerHTML = `
                        <div class="t-row c-up"><span>↑ <span class="v-vol"></span></span> <span class="v-pct"></span></div>
            ...
            if (infoDiv) {
                ...
                // A. 新增雷达比例条 (置于中间偏下，利用 margin-top: auto 往下推)
                ...
                if (!ratioBox) {
                    ...
                    ratioBox.innerHTML = `
                        <div class="gege-ratio-top">
                            <span class="v-port" style="color: #333;"></span>
                ...
                // 根据环境变量控制红蓝色
                rtPctNode.style.color = upRatio > CONFIG.ratioThreshold ? '#ff4c00' : '#0059fa';
                // 左红右蓝：内部元素是红色，父容器背景是蓝色
                ratioBox.querySelector('.gege-ratio-bar-inner').style.width = `${upRatio}%`;

                // B. 下行流量组件 (置于最底，紧贴雷达图下方)
                let downBox = infoDiv.querySelector('.gege-down-box');
                if (!downBox) {
                    downBox = document.createElement('div');
                    downBox.className = 'gege-down-box';
                    downBox.innerHTML = `
                        <div class="t-row c-down"><span>↓ <span class="v-vol"></span></span> <span class="v-pct"></span></div>
