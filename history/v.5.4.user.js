...
(function() {
    ...
    async function buildVirtualDOM(overlay) {
        try {
            ...
            instances.forEach(inst => {
                ...
                let itemHtml = `
                    <div class="col-md-12 col-xs-12 config-item" data-gege-mac="${mac}" style="background: #fff; margin-bottom: 10px; padding: 15px 10px; border-radius: 4px; border: 1px solid #eee;">
                        <div class="config-item-box" style="display: flex; align-items: stretch;">
                            <div class="col-md-5 col-xs-7 logo" style="width: 33%; display: flex; flex-direction: row;">
            ...
            overlay.innerHTML = `
                <div style="padding: 20px; max-width: 1200px; margin: 0 auto; min-height: 100%;">
                    <div id="config-list" class="config-list">
                        <div>
                            <div class="config-title" style="font-size: 16px; font-weight: bold; margin: 15px 0 10px; color: #333;">无线设备（2.4GHz）</div>
                            ${html2g || '<div style="color:#999; padding: 10px;">没有连接设备</div>'}
                        </div>
                        <div>
                            <div class="config-title" style="font-size: 16px; font-weight: bold; margin: 15px 0 10px; color: #333;">无线设备（5.2GHz）</div>
                            ${html5_2g || '<div style="color:#999; padding: 10px;">没有连接设备</div>'}
                        </div>
                        <div>
                            <div class="config-title" style="font-size: 16px; font-weight: bold; margin: 15px 0 10px; color: #333;">无线设备（5.8GHz）</div>
                            ${html5_8g || '<div style="color:#999; padding: 10px;">没有连接设备</div>'}
                        </div>
                        <div>
                            <div class="config-title" style="font-size: 16px; font-weight: bold; margin: 15px 0 10px; color: #333;">有线设备</div>
                            ${htmlWired || '<div style="color:#999; padding: 10px;">没有连接设备</div>'}
                        </div>
                    </div>
                </div>
            `;
    ...
    function injectGegeMenu() {
        ...
        if (liTag) {
            ...
            emojiSpan.style.cssText = 'font-size: 20px; margin-right: 10px; vertical-align: middle; display: inline-block; width: 24px; text-align: center;';
        ...
        document.addEventListener('click', function(e) {
            ...
            if (clickedWrapper.id === 'gege-menu-wrapper') {
                ...
                document.querySelectorAll('.menu_items li').forEach(li => li.classList.remove('is-active'));
                if(aTag) aTag.classList.add('router-link-exact-active', 'router-link-active');
                if(liTag) liTag.classList.add('is-active');
