...
abU=0,abD=0,
cln={};
for(let k in S.cls){let s=S.cls[k]||{};let cU=Math.max(0,(s.lU||0)-(s.uB||0)-(s.oU||0));let cD=Math.max(0,(s.lD||0)-(s.dB||0)-(s.oD||0));let cC=cI[k]||{};let pN=CONFIG.portMap[cC.iface]||cC.iface||"未知";cln[k]={up:cU,down:cD,status:s.aR?"off":pN,name:cC.name||k};LUp+=s.intUp;LDn+=s.intDn;hpU+=cU;hpD+=cD;}
for(let m in cI){let c=cI[m]||{};abU+=c.offUp||0;abD+=c.offDn||0;tOU+=c.offUp||0;tOD+=c.offDn||0;}
let ha_devs={};for(let k in cln){ha_devs[k]={up:cln[k].up,down:cln[k].down,status:cln[k].status,name:cln[k].name||k,ip:(cI[k]&&cI[k].ip)||"",raw_up:(cI[k]&&cI[k].offUp)||0,raw_down:(cI[k]&&cI[k].offDn)||0};}
GM_setValue('ha_snapshot',{timestamp:Date.now(),global:{wan_up:S.wTotUp,wan_down:S.wTotDn,lan_high_up:hpU,lan_high_down:hpD,lan_off_up:abU,lan_off_down:abD},devices:ha_devs});
...
const M_RX=/([a-fA-F0-9]{2}[:\-]){5}[a-fA-F0-9]{2}/;let oDC=Object.create(null);
if(!iPO){let aI=aC.querySelectorAll('.config-item');for(let n of aI){let mN=n.querySelector('.dev-number'),mM=mN?mN.textContent.match(M_RX):null;if(mM){oDC[mM[0].toLowerCase().replace(/-/g,':')]=n;}}}
for(let m in cI){
let it=null;
if(iPO){
it=aC.querySelector(`.gege-list-item[data-gege-mac="${m}"]`);
}else{
it=oDC[m];
}
if(!it)continue;
...
let pu=sU>0?(cC.upRate/sU*100):0,pd=sD>0?(cC.dnRate/sD*100):0,bU=enh.querySelector('.zte-bar-up'),bD=enh.querySelector('.zte-bar-down');
    ...
    Object.keys(window.gegeHiddenDevices).length>0)?'<span style="color: #ff4c00; font-size: 13px; font-weight: normal; margin-left: 10px; font-family: Consolas;">(哥哥科技：智能Mesh适配)</span>':'',h2='',h52='',h58='',hW='';(
        ...
        `style="padding: 20px; max-width: 1500px; margin: 0 auto; min-height: 100%;"><div id="gege-board-anchor"></div><div id="config-list" class="config-list gege-list-container"><div `+
        ...
        gegeEngineRunning=!0;try{const ts=Date.now(),wR=await fetch(`/?_type=vueData&_tag=vue_home_device_data_no_update_sess&IF_OP=refresh&_=${ts}`);if(
            !wR.ok)throw new Error();const wT=await wR.text(),wST=performance.now();if(!wT.includes('<OBJ_HOME_BASICINFO_ID>'))throw new Error();const wX=
              pr.parseFromString(wT,"text/xml");let bIN=wX.querySelector("OBJ_HOME_BASICINFO_ID Instance"),wI=bIN?pI(bIN):{},cDC=parseInt(
                  ...
                  `/?_type=vueData&_tag=vue_topo_data&_=${ts}`);if(!tR.ok)throw new Error();let tJ;try{tJ=JSON.parse(await tR.text());}catch(e){throw new Error("Topo Payload corrupted");}let mDC=tJ.agentlay1?.reduce((s,a)=>s+(parseInt(a.accdevCount)||0),0)||0;if(mDC!==window.gegeLastMeshDevCount){window.gegeLastMeshDevCount=mDC;window.
