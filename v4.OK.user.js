...
(function() {
    ...
    // ======== [2] 注入极客宽屏 CSS (像素级绝对对齐系统 + 完美V6 UI) ========
    ...
    style.innerHTML = `
        ...
        .config-item .dev-intro { flex: 1; display: flex !important; flex-direction: column; justify-content: flex-start; min-height: 100px; align-self: stretch !important; padding-bottom: 0 !important; margin-bottom: 0 !important; box-sizing: border-box; }
        .config-item .info { width: 27% !important; float: none !important; display: flex !important; flex-direction: column; justify-content: flex-start; padding: 0 10px !important; border-right: 1px solid #eee; padding-bottom: 0 !important; margin-bottom: 0 !important; box-sizing: border-box; font-family: Consolas, "Courier New", "PingFang SC", "Microsoft YaHei", sans-serif; }
        .config-item .speed { width: 40% !important; float: none !important; display: flex !important; flex-direction: column; justify-content: center; padding: 0 10px !important; box-sizing: border-box; }
        ...
        #zte-geek-board {
            ...
            font-family: Consolas, "Courier New", "PingFang SC", "Microsoft YaHei", sans-serif;
            box-shadow: 0 2px 8px rgba(0,0,0,0.05); font-size: 14px; display: flex; flex-direction: column; gap: 8px; color: #333;
        }
        ...
        /* 核心对齐法：推到底部并锁死 margin-bottom 为 0 消除高度差 */
        .gege-up-box, .gege-down-box { margin-top: auto !important; margin-bottom: 0 !important; width: 95%; }
        .gege-ratio-box { margin-top: 8px; width: 95%; margin-bottom: 6px; }

        /* 细条组件 - 吸收V6半透明温柔配色与字体，锁死 line-height */
        .t-row { font-size: 12px; font-weight: bold; margin-bottom: 3px; display: flex; justify-content: space-between; color: #666; font-family: Consolas, "Courier New", "PingFang SC", "Microsoft YaHei", sans-serif; line-height: 1.2; }
        ...
        /* 网速进度条 - 吸收V6字体与半透明背景色，彻底修复字体大小 bug */
        .zte-enhance-speed { display: flex; flex-direction: column; gap: 6px; width: 100%; font-family: Consolas, "Courier New", "PingFang SC", "Microsoft YaHei", sans-serif; }
        .zte-bar-wrap {
        ...
        /* 强制覆盖内部所有的 span 继承字体大小与加粗，拒绝大小眼 */
        ...
        .zte-bar-up::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; z-index: -1; background: rgba(255, 76, 0, 0.15); width: var(--p-up, 0%); transition: width 0.5s ease-out; }
        .zte-bar-down::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; z-index: -1; background: rgba(0, 89, 250, 0.15); width: var(--p-down, 0%); transition: width 0.5s ease-out; }
    `;
        ...
        try {
            ...
            clientNodes.forEach(node => {
                ...
                if (dev.MACAddress) {
                    ...
                    clientsInfo[mac] = { up: up, down: down, interface: dev.Interface || "" };
                    curSumUp += up;
                    curSumDown += down;
                }
            });
        ...
        deviceItems.forEach(item => {
            ...
            if (devIntro) {
                ...
                if (!box) {
                    ...
                    box.innerHTML = `<div class="t-row c-up"><span>↑ <span class="v-vol"></span></span><span><span class="v-pct"></span>%</span></div><div class="zte-thin-bar"><div class="zte-thin-bar-inner up"></div></div>`;
                    devIntro.appendChild(box);
                }
                let p = State.wanUpTraffic>0?((cS.upTraffic/State.wanUpTraffic)*100).toFixed(1):"0.0";
                box.querySelector('.v-vol').textContent = formatVolume(cS.upTraffic);
            ...
            if (info) {
                ...
                if (!rBox) {
                    ...
                    rBox.innerHTML = `<div class="gege-ratio-top"><span class="v-port" style="color: #000;"></span><span class="v-rt-pct"></span></div><div class="gege-ratio-bar"><div class="gege-ratio-bar-inner"></div></div>`;
                    info.appendChild(rBox);
                }
                let totalV = cS.upTraffic + cS.downTraffic;
                let uR = totalV > 0 ? (cS.upTraffic/totalV*100) : 0;
                ...
                rtPct.textContent = uR.toFixed(1) + '%';
                rtPct.style.color = uR > CONFIG.ratioThreshold ? '#ff4c00' : '#0059fa';
                rBox.querySelector('.gege-ratio-bar-inner').style.width = uR + '%';

                let dBox = info.querySelector('.gege-down-box');
                if (!dBox) {
                    dBox = document.createElement('div'); dBox.className = 'gege-down-box';
                    dBox.innerHTML = `<div class="t-row c-down"><span>↓ <span class="v-vol"></span></span><span><span class="v-pct"></span>%</span></div><div class="zte-thin-bar"><div class="zte-thin-bar-inner down"></div></div>`;
                    info.appendChild(dBox);
                }
                let dp = State.wanDownTraffic>0?((cS.downTraffic/State.wanDownTraffic)*100).toFixed(1):"0.0";
                dBox.querySelector('.v-vol').textContent = formatVolume(cS.downTraffic);
            ...
            if (speed) {
                ...
                if (!enh) {
                    // 这里完美复刻了 V6 的代码，去掉了污染样式的 <small>
                    enh = document.createElement('div'); enh.className = 'zte-enhance-speed';
                    enh.innerHTML = `
                        <div class="zte-bar-wrap zte-bar-up"><span class="v-val"></span><span class="v-pct"></span></div>
                        <div class="zte-bar-wrap zte-bar-down"><span class="v-val"></span><span class="v-pct"></span></div>
                    `;
                ...
                bU.querySelector('.v-pct').textContent = wanUp > 0 ? pu.toFixed(1) + '%' : (cCur.up > 0 ? ">100%" : "0.0%");

                bD.style.setProperty('--p-down', Math.min(pd, 100)+'%');
                bD.querySelector('.v-val').textContent = `🔽 ${formatBytes(cCur.down)}`;
                bD.querySelector('.v-pct').textContent = wanDown > 0 ? pd.toFixed(1) + '%' : (cCur.down > 0 ? ">100%" : "0.0%");
            }
        });
    }
