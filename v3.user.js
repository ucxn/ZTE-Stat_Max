...
// @version      6.0.0
// @description  横向分布降低行高，1000ms超频微秒积分，纯公网基准，丝滑无痕
// @include      http://10.*
...
(function() {
    ...
    // ======== [2] 注入极客宽屏 CSS (横向布局优化版) ========

    const style = document.createElement('style');
    style.innerHTML = `
        /* 强制调整中兴原有栅格，确保横向空间充足 */
        .config-item .logo { width: 32% !important; }
        .config-item .info { width: 28% !important; padding: 0 5px !important; font-family: Consolas, monospace; }
        .config-item .speed { width: 40% !important; padding: 0 10px !important; }
        ...
        /* 极简细条微型组件 */
        .gege-up-box, .gege-down-box { margin-top: 4px; width: 90%; }
    ...
    function renderUI(wanUp, wanDown, sumUp, sumDown, lanUpVol, lanDownVol, clientsInfo) {
        ...
        deviceItems.forEach(item => {
            ...
            if (infoDiv) {
                const originalIps = Array.from(infoDiv.querySelectorAll('.dev-ip:not(.gege-down-box *)'));
                if (originalIps.length > 1) {
                    for(let i = 1; i < originalIps.length; i++) originalIps[i].style.display = 'none';
                }
                const originalMacs = Array.from(infoDiv.querySelectorAll('.dev-number:not(.gege-down-box *)'));
    ...
    // ======== [5] 1秒超频点火启动 ========
    // 脱离官方轮询节奏，独立实现 1000ms 刷新率，极高精度保障
    setInterval(() => {
